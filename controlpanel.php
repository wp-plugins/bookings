...
function bookings_options() {
	...
	if (!get_option('bookings_region')) update_option('bookings_region', zingiri::findNearestServer($bookingsRegions));
	...
	$bookings_options[100]=array("name" => "Settings","type" => "heading","desc" => "This section customizes the way the Bookings plugin works.");
	$bookings_options[110]=array("name" => "API Key","desc" => 'This plugin uses remote web services to provide mailing list functionality. This API key has been automatically generated for you. Once you click on Install, the API key, in combination with your web site address <strong>' . home_url() . '</strong> will create an account on our servers allowing the plugin to access the remote web services.<br />The combination of API key and your web site address uniquely identifes you so please make sure to keep it in a safe place.',"id" => $bookings_shortname . "_key","type" => "text");
	if (0) $bookings_options[111]=array("name" => "Secret","desc" => 'This is your secret or password, it is uniquely linked to your Bookings account, make sure to keep it in a safe place.',"id" => $bookings_shortname . "_secret","type" => "text");
	$bookings_options[120]=array("name" => "License Key","desc" => 'If you wish to make use of the <strong>Bookings Pro</strong> features, enter your license key here. You can purchase a license key <a href="http://www.zingiri.com/portal/?ccce=cart&a=add&pid=121" target="blank">here</a>. The Pro version provides additional functionality and has no limits to the number of bookings and schedules you can use.',"id" => $bookings_shortname . "_lic","type" => "text");
	$bookings_options[130]=array("name" => "Region","desc" => "Region you are connected to.","value" => $bookingsRegions[get_option("bookings_region")][0],"type" => "info");
	$p=parse_url(home_url());
	
	if (0) $bookings_options[135]=array("name" => "Showcase Your Site","desc" => "If you choose to participate in our showcase, we will list your site <strong style=\"color:blue\">http://" . $p['host'] . "</strong> with the description '<strong style=\"color:blue\">" . get_bloginfo('name') . ' - ' . get_bloginfo('description') . "</strong>' on our site. This is a way to show your support for this plugin and a bit of free advertising for you. Please don't join the program until your site is ready.","id" => $bookings_shortname . "_showcase","type" => "checkbox");
	if (0) $bookings_options[140]=array("name" => "Debug Mode","desc" => "If you have problems with the plugin, activate the debug mode to generate a debug log for our support team","id" => $bookings_shortname . "_debug","type" => "checkbox");
	...
	if (0) $bookings_options[160]=array("name" => "Admin capability","desc" => "Choose the required capability for managing settings, schedules and resources. It is recommended to use an admin type of capability, such as 'Manage options'","id" => $bookings_shortname . "_admin_cap","options" => $allCaps,"std" => 'manage_options',"type" => "selectwithkey");
	if (0) $bookings_options[161]=array("name" => "User capability","desc" => "Choose the required capability for managing bookings. It is recommended to use an editor type of capability, such as 'Edit posts'","id" => $bookings_shortname . "_user_cap","options" => $allCaps,"std" => 'edit_posts',"type" => "selectwithkey");
	
	if (defined('BOOKINGS_LIVE')) bookings_live_options($bookings_options);
	
	$bookings_options[170]=array("name" => "Custom styles","desc" => "You can use css custom styles here to overwrite the Bookings styles.","id" => $bookings_shortname . "_css","type" => "textarea");
	
	$bookings_options[999]=array("name" => "Before you install","type" => "heading","desc" => '<div style="text-decoration:underline;display:inline;font-weight:bold">IMPORTANT:</div> Bookings uses web services stored on Zingiri\'s servers. In doing so, personal data is collected and stored on our servers. 
...
function bookings_admin() {
	...
	if (get_option('bookings_key') && current_user_can(BOOKINGS_ADMIN_CAP)) {
		...
		echo '<p>To administer Bookings, please login <a href="' . bookings_url(false) . 'index.php?apikey=' . get_option('bookings_key') . '" target="_blank">here</a>.</p>';
